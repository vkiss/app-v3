...
export default function generateSvg ( data = { width: sessionStorage.getItem( "pixelEditor_config_x" ), height: sessionStorage.getItem( "pixelEditor_config_y" ) }, pixelSheet = sessionStorage.getItem( "pixelEditor_canvasBoard" ) ) {
  const ns = "http://www.w3.org/2000/svg";
  const svg = document.createElementNS( ns, "svg" );
  svg.setAttributeNS( null, "width", data.width );
  svg.setAttributeNS( null, "height", data.height );

  for( const pixel of translateASCIICode( pixelSheet ).map ) {
